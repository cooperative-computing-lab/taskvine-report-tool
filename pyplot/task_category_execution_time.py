...
def plot_task_time(show=True, log_scale=False, category=None, cdf=False):
    ...
    for i, task_done_csv in enumerate(TASK_DONE_CSV_FILES):
        ...
        task_done_df['execution_time'] = task_done_df['time_worker_end'] - task_done_df['time_worker_start']
        ...
        if not cdf:
            ...
            ax.set_ylabel("Execution Time (s)", fontsize=PLOT_SETTINGS["label_fontsize"])
            ...
            cdf = np.arange(1, len(execution_times_sorted) + 1) / len(execution_times_sorted)
        ...
        ax.grid(True, linestyle='--', linewidth=PLOT_SETTINGS['grid_linewidth'], alpha=PLOT_SETTINGS['grid_alpha'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log-scale', action='store_true', help="Use log scale in x-axis.")
    parser.add_argument('--category', type=str, help="Filter tasks by categories.", required=True)
    parser.add_argument('--cdf', action='store_true', help="Plot the CDF of execution time.")
    args = parser.parse_args()

    plot_task_time(show=False, log_scale=args.log_scale, category=args.category, cdf=args.cdf)
