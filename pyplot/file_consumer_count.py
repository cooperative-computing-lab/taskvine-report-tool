...
def plot_file_consumer_count(show=True):
    ...
    for i, file_info_csv in enumerate(FILE_INFO_CSV_FILES):
        ...
        df['consumers_count'] = df['consumers'].apply(lambda x: len(eval(x)) if pd.notna(x) else 0)

        df_count = df['consumers_count'].value_counts().sort_index()

        ax = fig.add_subplot(gs[0, i])

        bar_plot = sns.barplot(x=df_count.index, y=df_count.values, color='steelblue', ax=ax, width=0.5)

        for p in bar_plot.patches:
            ax.text(p.get_x() + p.get_width() / 2, p.get_height(), f'{int(p.get_height())}', 
                    ha='center', va='bottom', fontsize=10, color='red')

        ax.set_xlabel('Number of Consumers', fontsize=PLOT_SETTINGS["label_fontsize"])
        ax.set_ylabel('Number of Files', fontsize=PLOT_SETTINGS["label_fontsize"])
        ...
        ax.grid(True, color='#d3d3d3', linewidth=PLOT_SETTINGS['grid_linewidth'], alpha=PLOT_SETTINGS['grid_alpha'])

    plt.savefig(os.path.join(SAVE_TO, 'file_consumer_count.png'), bbox_inches='tight')
