...
def plot_task_states_over_time(show=True):
    ...
    gs = GridSpec(1, num_logs, figure=fig, wspace=0.3, left=0.15, right=0.9, top=0.8, bottom=0.1)
    ...
    for i, manager_info_path in enumerate(MANAGER_INFO_CSV_FILES):
        ...
        for current_time in time_range:
            ready_tasks = task_df[
                ...
                task_df['when_ready'].notna() & task_df['when_input_transfer_ready'].notna()
            ]
            committing_tasks = task_df[
                (task_df['when_input_transfer_ready'] <= current_time) &
                (task_df['time_worker_start'] > current_time) &
                task_df['when_input_transfer_ready'].notna() & task_df['time_worker_start'].notna()
            ]
    ...
    for i, (time_range, ready_counts, committing_counts, running_counts, waiting_retrieval_counts) in enumerate(task_counts):
        ...
        ax.set_ylabel('Number of Tasks', fontsize=PLOT_SETTINGS['label_fontsize'])
    ...
    fig.legend(lines, labels, loc='upper center', ncol=4, fontsize=PLOT_SETTINGS['legend_fontsize'])

    plt.savefig(os.path.join(SAVE_TO, 'task_states_over_time.png'), bbox_inches='tight')
    if show:
        plt.tight_layout()
        plt.show()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-ready', action='store_true', help="No ready tasks.")
