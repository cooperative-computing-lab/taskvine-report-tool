...
def plot_individual_worker_disk_usage(add_peak_line_and_text=True, show=True, plot_manager=False, plot_file_count=False):
    ...
    for i, csv_file in enumerate(DISK_USAGE_CSV_FILES):
        ...
        ax.set_ylabel('Worker Storage Consumption (GB)', fontsize=PLOT_SETTINGS["label_fontsize"])
        ...
        if plot_manager:
            ...
            if os.path.exists(manager_csv_file):
                ...
                df_manager['accumulated_disk_usage_gb'] = df_manager['accumulated_disk_usage(MB)'] / 1024

                # Ensure the first point is at zero
                first_point = pd.DataFrame({'adjusted_time': [0], 'accumulated_disk_usage_gb': [0]})
                df_manager = pd.concat([first_point, df_manager], ignore_index=True)
                ...
                ax2.set_ylabel('Manager Storage Consumption (GB)', fontsize=PLOT_SETTINGS["label_fontsize"])
                ax2.set_ylim(0, global_max_manager_disk_usage_gb * 1.1)
                ax2.tick_params(axis='y', labelsize=PLOT_SETTINGS["tick_fontsize"])

                ax2.legend(loc='upper left', fontsize=PLOT_SETTINGS["legend_fontsize"])
            ...
            ax2.plot(df['adjusted_time'], df['file_count'], color='green', linestyle=':', linewidth=1.5, alpha=0.7, label='File Count')
            ax2.set_ylabel('File Count', fontsize=PLOT_SETTINGS["label_fontsize"])
            ax2.set_ylim(0, global_max_file_count * 1.1)
            ax2.tick_params(axis='y', labelsize=PLOT_SETTINGS["tick_fontsize"])

            ax2.legend(loc='upper left', fontsize=PLOT_SETTINGS["legend_fontsize"])

    plt.savefig(os.path.join(SAVE_TO, 'file_accumulation_worker.png'), bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--manager', action='store_true', help="Plot manager storage consumption on right y-axis.")
    parser.add_argument('--file-count', action='store_true', help="Plot file count on a secondary y-axis.")
    args = parser.parse_args()

    plot_individual_worker_disk_usage(add_peak_line_and_text=not args.no_peak, show=True, plot_manager=args.manager, plot_file_count=args.file_count)
