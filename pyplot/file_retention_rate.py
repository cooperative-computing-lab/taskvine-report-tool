...
def generate_file_retention_time_csv(file_info_csv, file_retention_time_csv):
    ...
    manager_info_df = pd.read_csv(file_info_csv.replace(
        'file_info.csv', 'manager_info.csv'))
    ...
    file_retention_time_df = pd.DataFrame(
        data, columns=['filename', 'size(MB)', 'file_retention_time', 'file_retention_rate'])
...
def plot_file_retention_time(show=True, x_unit='time', y_axis='file_index'):
    ...
    gs = GridSpec(1, num_logs, figure=fig, wspace=0.3,
                  left=0.15, right=0.9, bottom=0.1)
    ...
    for csv_file in FILE_INFO_CSV_FILES:
        file_retention_time_csv = csv_file.replace(
            'file_info.csv', 'file_retention_time.csv')
    ...
    for i, csv_file in enumerate(FILE_INFO_CSV_FILES):
        ...
        ax.set_yticks(np.linspace(0, max_file_index,
                      PLOT_SETTINGS["yticks_count"]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--by_rate', action='store_true',
                        help='Plot by file retention rate if set, otherwise by file retention time')
