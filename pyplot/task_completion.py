...
def plot_cumulative_task_completion_for_multiple_logs(show=True):
    ...
    for i, (manager_info_path, task_done_path) in enumerate(zip(MANAGER_INFO_CSV_FILES, TASK_DONE_CSV_FILES)):
        if not os.path.exists(manager_info_path) or not os.path.exists(task_done_path):
            print(f"File {manager_info_path} or {task_done_path} does not exist.")
            continue
        ...
        task_completion_counts = task_done_df.groupby('adjusted_time_worker_end').size()
        ...
        ax.set_ylabel('Completed Tasks', fontsize=PLOT_SETTINGS['label_fontsize'])
    ...
    plt.savefig(os.path.join(SAVE_TO, 'cumulative_tasks_completed.png'), bbox_inches='tight')
    if show:
        plt.tight_layout()
        plt.show()
