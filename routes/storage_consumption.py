...
storage_consumption_bp = Blueprint('storage_consumption', __name__, url_prefix='/api')
    ...
    try:
        ...
        for file in files.values():
            for transfer in file.transfers:
                ...
                time_in = float(transfer.time_start_stage_in - runtime_state.MIN_TIME)
                time_out = float(transfer.time_stage_out - runtime_state.MIN_TIME)
        ...
        for destination in list(data['worker_storage_consumption'].keys()):
            ...
            if worker.time_connected and worker.time_disconnected:
                for time_connected, time_disconnected in zip(worker.time_connected, worker.time_disconnected):
                    ...
                    time_end = float(time_disconnected - runtime_state.MIN_TIME)
